...
def test_Long_serialization():
    ...
    assert s.Long.serialize(18446744073709551615) == int(18446744073709551615).to_bytes(8, "big")
...
def test_serialize_List_of_Strings():
    x = ["one", "two", "three"]
    x_bytes = len(x).to_bytes(4, "big") + b"".join([len(x).to_bytes(2, "big") + bytes(x, "utf-8") for x in x])
...
def test_serialize_object():
    ...
    x = X()
    x_bytes = x.one.to_bytes(8, "big") + int(3).to_bytes(4, "big") + b"".join([len(x).to_bytes(2, "big") + bytes(x, "utf-8") for x in x.two]) + x.three.to_bytes(1, "big")
